...
const getOneQuestion = async (filters) => {
  try {
    const question = await Question.findOne(filters);
    return question;
  } catch (error) {
  ...
  try {
    ...
    if (!question) {
      return Promise.reject(
        new Error("No Question with the given questionId " + questionId)
      );
    }
...
const updateTagAndCounter = async (data) => {
  ...
  if (subjectCount.hasOwnProperty(mcqSubject)) {
    ...
    if (subjectCount[mcqSubject][mcqTopic] !== undefined) {
      ...
      subjectCount[mcqSubject][mcqTopic] = [
        data.difficulty === "Easy" ? 1 : 0,
        data.difficulty === "Medium" ? 1 : 0,
        data.difficulty === "Hard" ? 1 : 0,
      ];
    }
  } else {
    const newTopic = {
...
const getCorrectOptionsTopicsAndSubjectsMap = async (questionsList) => {
  try {
    ...
    for (let i = 0; i < questions.length; i++) {
      questionIdToCorrectOptionMap.set(
