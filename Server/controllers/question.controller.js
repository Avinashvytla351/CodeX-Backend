...
exports.create = async (req, res) => {
  ...
  if (req.body.questionName === undefined) {
    return responseUtil.sendResponse(
      ...
      false,
      null,
      "Question name can not be empty",
      400
    );
  }
  var nameCheck = req.body.questionName;
  nameCheck = nameCheck.split(" ");
  var name = "";
  nameCheck.forEach((item) => {
    if (item != " ") {
      name += item[0].toUpperCase() + item.slice(1).toLowerCase();
    }
  });
  try {
...
exports.createMcqQuestion = async (req, res) => {
  ...
  for (var i = 0; i < req.body.length; i++) {
    try {
      ...
      const questionId = req.body[i].isMcq
        ? mcqCode + (Number(counter.mcqCount) + 1).toString()
        : questionCode + (Number(counter.questionCount) + 1).toString();
      const question = new Question({
...
exports.getAllQuestions = async (req, res) => {
  try {
    const filters =
      req.query.queryString !== undefined
        ? req.query.queryString.split(",")
        : [];
    const questions = await questionUtil.getAllQuestions(
      questionUtil.decodeFilters(filters),
      req.params.isMcq === "mcq"
    );
    return responseUtil.sendResponse(
      res,
      true,
  ...
  try {
    if (req.params.questionId === undefined) {
      return responseUtil.sendResponse(
        ...
        false,
        null,
        "questionId is not provided",
        400
      );
    }
    const question = await questionUtil.getOneQuestion(
      req.params.questionId,
      req.isAdmin ? {} : questionUtil.decodeFilters(filters)
    );
    return responseUtil.sendResponse(
      res,
      true,
...
exports.updateQuestion = async (req, res) => {
  ...
  try {
    const updatedQuestion = await questionUtil.updateQuestion(
      req.params.questionId,
      req.body
    );
    return responseUtil.sendResponse(
    ...
    return responseUtil.sendResponse(
      ...
      false,
      null,
      "Error while updating Question with questionId " + req.body.questionId,
      500
    );
  }
};
...
exports.deleteQuestion = async (req, res) => {
  ...
  try {
    const deletedQuestion = await questionUtil.deleteQuestion(
      req.params.questionId
    );
    return responseUtil.sendResponse(
      res,
    ...
    return responseUtil.sendResponse(
      ...
      false,
      null,
      "Error while deleting Question with questionId " + req.params.questionId,
      500
    );
  }
};
...
exports.deleteMultiple = async (req, res) => {
  if (req.body.questionIds === undefined) {
    return responseUtil.sendResponse(
      ...
      false,
      null,
      "QuestionIds cannot be empty",
      400
    );
  }
  try {
    const deletedQuestions = await questionUtil.deleteMultipleQuestions(
  ...
  if (contest.isManual) {
    if (contest.questionsList.length === 0) {
      return Promise.reject(
        new Error("Contest does not have any questions (Manual Contest)")
      );
    }
    ...
    if (contest.sets.length === 0) {
      return Promise.reject(
        new Error("Contest does not have any sets (Multiple Set Contest)")
      );
    }
  ...
  try {
    ...
    if (!contest) {
      return responseUtil.sendResponse(
        ...
        false,
        null,
        "Contest could not fetched due with contestId " + req.params.contestId,
        400
      );
    }
    const participationId =
      req.body.username.toLowerCase() + req.params.contestId;
    if (contest.isMcqContest === true) {
      ...
      const participation = await participationUtil.createParticipation(
        participationId,
        participationData
      );
      const questions = await questionUtil.getMultipleQuestions(
        contest.questionsList,
        questionUtil.decodeFilters(req.filters)
      );
      return responseUtil.sendResponse(
        res,
        true,
        { participation, questions },
        "Questions and Participation fetched successfully ",
        200
      );
    } else {
      var participationData = {
      ...
      const exisitingParticipation =
        await participationUtil.getOneParticipation(participationId);
      var questionsList = [];
